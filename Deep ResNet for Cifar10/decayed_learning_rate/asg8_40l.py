...
def main():
  ...
  for epoch in range(args.start_epoch, args.epochs):
    ...
    optimizer = AccSGD(net.parameters(),current_learning_rate , kappa = 1000.0, xi = 10.0)
    optimizer.zero_grad()
    optimizer.step()      
    ...
    }, is_best, args.save_path, 'asg8_40lcheck.pth.tar')

    # measure elapsed time
    epoch_time.update(time.time() - start_time)
    start_time = time.time()
    recorder.plot_curve( os.path.join(args.save_path, 'asg8_40l.png') )

  log.close()

# train function (forward, backward, update)
def train(train_loader, model, criterion, optimizer, epoch, log, lastloss):
  ...
  return top1.avg, losses.avg, loss_e
...
def save_checkpoint(state, is_best, save_path, filename):
  ...
  if is_best:
    bestname = os.path.join(save_path, 'asgl8_40model.pth.tar')
...
#   return lr
# =============================================================================
def adjust_learning_rate(optimizer, epoch, gammas, schedule,loss_e):
  """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
  lr = args.learning_rate 
  #assert len(gammas) == len(schedule), "length of gammas and schedule should be equal"
  lr=0.3
