...
def main():
  ...
  optimizer = AccSGD(net.parameters(), state['learning_rate'], kappa = 10000.0, xi = 20.0)
  optimizer.zero_grad()
  ...
  for epoch in range(args.start_epoch, args.epochs):
      ...
      #optimizer = torch.optim.SGD(net.parameters(), current_learning_rate, momentum=state['momentum'],
                  #weight_decay=state['decay'], nesterov=True)
      optimizer = AccSGD(net.parameters(),current_learning_rate , kappa = 10000.0, xi = 20.0)
      optimizer.zero_grad()
      optimizer.step()            
    ...
    #val_acc,   val_los   = extract_features(test_loader, net, criterion, log)
      ...
      }, is_best, args.save_path, 'asg8_40lcheck1k.pth.tar')

    # measure elapsed time
      epoch_time.update(time.time() - start_time)
      start_time = time.time()
      recorder.plot_curve( os.path.join(args.save_path, 'asg8_40l1k.png') )
...
def save_checkpoint(state, is_best, save_path, filename):
  ...
  if is_best:
    bestname = os.path.join(save_path, 'asg8_40lmodel1k.pth.tar')
